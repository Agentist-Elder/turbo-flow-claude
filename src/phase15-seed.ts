...
type GoalPhase = (typeof GOAP_PHASES)[number];
 ...
 */
function hashVector(seed: string): Float32Array {
  const buf: number[] = [];
  let counter = 0;
  ...
  while (buf.length < DIMENSIONS) {
    ...
    for (const byte of h) {
      if (buf.length >= DIMENSIONS) break;
 ...
 */
function perturbedVector(centroid: Float32Array, seed: string, alpha = 0.9): Float32Array {
  ...
  for (let i = 0; i < blended.length; i++) blended[i] /= mag;
  return blended;
}
...
function generatePhasePatterns(phase: GoalPhase): SeedEntry[] {
