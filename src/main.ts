...
/**
 * Simplified type for the HuggingFace feature-extraction pipeline instance.
 * The full Awaited<ReturnType<typeof pipeline>> union exceeds tsc's complexity
 * limit (TS2590). We only ever call the extractor for feature-extraction with
 * mean pooling, which always returns { data: Float32Array }.
 */
...
export async function runGoal(
  ...
  try {
    ...
    throw new Error(
      'STARTUP ASSERTION FAILED: ruvbot-clean-reference.db not found at ' +
      cleanRefDbPath + '\n' +
  ...
  const semanticAuditFn: AuditFn | null = extractor
