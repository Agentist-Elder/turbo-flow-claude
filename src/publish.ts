...
async function checkCosign(): Promise<string> {
  const candidates = [
  ...
  try {
    ...
    throw new Error(
      `cosign not found. Install it first:\n\n` +
...
function parseLogIndex(bundleJson: string): { logIndex: number | null; integratedTime: string | null } {
  try {
    ...
    const entries = (vm?.['tlogEntries'] ?? bundle['tlogEntries']) as unknown[] | undefined;
    ...
    const rb = bundle['rekorBundle'] as Record<string, unknown> | undefined;
    const payload = rb?.['Payload'] as Record<string, unknown> | undefined;
  ...
  if (m) return { logIndex: Number(m[1]), integratedTime: null };

  return { logIndex: null, integratedTime: null };
}

function rekorUrl(logIndex: number): string {
