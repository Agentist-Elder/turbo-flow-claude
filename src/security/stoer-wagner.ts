...
export interface MicroGraphEdge {
  u: number;      // source node index (0-based)
  v: number;      // target node index (0-based)
  weight: number; // positive edge weight
}
 ...
 */
export function stoerWagnerMinCut(n: number, edges: MicroGraphEdge[]): number {
  ...
  while (active.length > 1) {
    ...
    for (let i = 0; i < active.length; i++) {
      ...
      for (const v of active) {
        if (!inA[v]) key[v] += w[maxNode][v];
      }
    }

    // Cut of the phase: total weight of edges from t to A \ {t}.
    if (key[t] < minCut) minCut = key[t];
 ...
 */
export function buildStarGraph(cosineDistances: number[]): {
  n: number;
  edges: MicroGraphEdge[];
} {
  const k = cosineDistances.length;
  if (k === 0) return { n: 1, edges: [] };

  const edges: MicroGraphEdge[] = cosineDistances.map((d, i) => ({
    u: 0,
    v: i + 1,
 ...
 */
export function localMinCutLambda(cosineDistances: number[]): number {
  if (cosineDistances.length === 0) return 0;
