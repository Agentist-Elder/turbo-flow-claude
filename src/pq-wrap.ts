...
async function loadKeypair(seedFile: string = DEFAULT_SEED_FILE): Promise<Keypair> {
  const seedHex = (await readFile(seedFile, 'utf8')).trim();
  if (seedHex.length !== 64) throw new Error(`Invalid seed in ${seedFile}: expected 64 hex chars`);
  const seed = fromHex(seedHex);
  const { secretKey, publicKey } = ml_dsa65.keygen(seed);
  return { secretKey, publicKey, seedHex };
}

async function saveSeed(seedHex: string, seedFile: string = DEFAULT_SEED_FILE): Promise<void> {
...
export interface WrapManifest {
  algorithm: string;
  bundle_sha3_256: string;  // 'none' if bundle not yet created
  document_sha3_256: string;
  tsa_sha3_256: string;     // 'none' if .tsa.json does not exist
  wrapped_at: string;
}
 ...
 */
export async function wrapDocument(
  docPath: string,
  keypair: Keypair,
): Promise<PQWrapOutput> {
...
export interface VerifyResult {
  ...
  reason?: string;
  manifest?: WrapManifest;
  currentHashes?: { document: string; tsa: string; bundle: string };
}

/**
 * Verify an existing .pq.json wrapper against the current state of the document.
 */
export async function verifyWrapper(docPath: string): Promise<VerifyResult> {
  ...
  let output: PQWrapOutput;
  try {
    output = JSON.parse(await readFile(pqPath, 'utf8')) as PQWrapOutput;
  } catch (e) {
    return { valid: false, reason: `Cannot parse .pq.json: ${(e as Error).message}` };
  }
