...
function decodePayloads(text) {
    ...
    let out = text.replace(/\\x([0-9A-Fa-f]{2})/g, (_, hex) => String.fromCharCode(parseInt(hex, 16)));
    // URL encoding: %20 -> ' '
 ...
 */
export function textToVector(text, dimensions = 384) {
    ...
    async scan(input) {
        ...
        for (const hit of results) {
            ...
            if (hit.score < this.config.suspiciousThreshold) {
                matchCount++;
                const severity = hit.metadata?.severity ?? 0.5;
            ...
            for (let i = 0; i < patterns.length; i++) {
                for (let j = i + 1; j < patterns.length; j++) {
                    ...
                    for (let d = 0; d < dims; d++)
                        dot += patterns[i].vector[d] * patterns[j].vector[d];
                    totalDist += 1 - dot;
                    pairs++;
                }
            }
